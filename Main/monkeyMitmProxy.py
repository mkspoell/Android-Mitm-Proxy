...
fileName = './' + logPath + '/loggedConnections_' + time.asctime( time.localtime(time.time()) )
    ...
    def log_message(self, format, *args):
        ...
        if potentialAddress.lower().startswith(headers):
            ...
            if 'CUSTOM-LOGGING' in potentialAddress:
                ...
                connectionList.append(potentialAddress.split(' ')[1].split('/')[-1])
...
if __name__ == '__main__':
    ...
    if not argv[1:]:
        ...
        proxy = MitmProxy(RequestHandlerClass=MitmProxyHandler, ca_file=argv[1])
    try:
